    ...
    , _active(false)
{
    ...
    _message_changed_connection = _message_stack->connectChanged(
        [this](Inkscape::MessageType type, const gchar *message) { onStatusMessage(type, message); });
}
...
void SPDesktop::set_display_width(Geom::Rect const &rect, Geom::Coord border)
{
    ...
    set_display_area(Geom::Rect(Geom::Point(rect.left(), center_y), Geom::Point(rect.width(), center_y)), border);
}
...
void SPDesktop::schedule_zoom_from_document()
{
    ...
    _schedule_zoom_from_document_connection = canvas->signal_draw().connect(
        [this](Cairo::RefPtr<Cairo::Context> const &) {
 ...
 */
void SPDesktop::scroll_relative_in_svg_coords(double dx, double dy)
{
...
void SPDesktop::setDocument(SPDocument *doc)
{
    ...
    _document_uri_set_connection =
        document->connectFilenameSet([this](const gchar *filename) { onDocumentFilenameSet(filename); });
...
void SPDesktop::showNotice(Glib::ustring const &msg, unsigned timeout)
{
...
void SPDesktop::onDocumentFilenameSet(gchar const *filename)
{
 ...
 */
static bool _drawing_handler(Inkscape::CanvasEvent const &event, Inkscape::DrawingItem *drawing_item,
                             SPDesktop *desktop)
{
...
sigc::connection
SPDesktop::connect_control_point_selected(const sigc::slot<void(void *, Inkscape::UI::ControlPointSelection *)> &slot)
{
    return _control_point_selected.connect(slot);
}

sigc::connection
SPDesktop::connect_text_cursor_moved(const sigc::slot<void(void *, Inkscape::UI::Tools::TextTool *)> &slot)
{
