...
//    _mainbox->set_visible(true); //TODO(David): Maybe change to false instead comment
    add(*_mainbox);

    // Desktop widget (=> MultiPaned)
    _desktop_widget = new SPDesktopWidget(this, _document);
    _desktop_widget->window = this;
//    _desktop_widget->set_visible(true); //TODO(David): Maybe change to false instead comment
    _desktop = _desktop_widget->desktop;

    // =================== Actions ===================
    // After canvas has been constructed.. move to canvas proper.
    add_actions_canvas_mode(this);      // Actions to change canvas display mode. //TODO(David): Maybe move bellow canvas_transform
    ...
    if (connection) {
        std::string document_action_group_name =
            _app->gio_app()->get_dbus_object_path() + "/document/" + std::to_string(get_id());
...
void InkscapeWindow::setup_view()
{
    ...
    //       the window, but a call to set_visible(false) causes Inkscape to just exit since the migration to
    //       Gtk::Application
//    set_visible(true); //TODO(David): Maybe set it to false instead comment 
//
...
bool InkscapeWindow::on_key_press_event(GdkEventKey *event)
{
    if constexpr (false) {
        std::cout << "InkscapeWindow::on_key_press_event: GDK_KEY_PRESS: " << std::hex
                  << " hardware: " << event->hardware_keycode << " state: " << event->state
